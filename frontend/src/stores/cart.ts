...
export const useCartStore = defineStore("cart", () => {
  ...
  const grandTotal = computed(() => serverGrandTotal.value ?? subtotal.value);

  const roundedTotal = computed(
    () => serverRoundedTotal.value ?? grandTotal.value,
  );
  ...
  function updateQty(
    ...
    const checkQty =
      availableQty !== undefined ? availableQty : item.actual_qty;
    ...
    if (
      ...
      const otherCartQty = items.value
        ...
        .reduce((sum, i) => sum + i.qty, 0);
      if (qty + otherCartQty > checkQty) {
        return __("Not enough stock. Available: {0}", [String(checkQty)]);
      }
    }
  ...
  function updateItemDiscount(index: number, discount: number) {
    items.value[index].discount_percentage = Math.min(
      Math.max(discount, 0),
      100,
    );
  ...
  async function calculateTaxes(posProfile?: string, customer?: string) {
    ...
    const cust =
      customer || customerStore.customer?.name || settings.posProfile?.customer;
  ...
  function applyPricingRuleData(
    ...
    for (const pr of pricingRules) {
      const cartItem = items.value.find(
        (i) => i.item_code === pr.item_code && !i.is_free_item,
      );
      if (cartItem) {
