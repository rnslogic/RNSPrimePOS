...
import { validateOfflineSecurity } from "@/utils/security";
...
export const usePaymentStore = defineStore("payment", () => {
  ...
  function initializePayments(
    ...
    activePaymentMethod.value =
      defaultMethod?.mode_of_payment ||
      paymentMethods[0]?.mode_of_payment ||
      "Cash";
  ...
  async function submitInvoice(
    ...
    try {
      ...
      // Delete the held order (draft) if we were resuming one
      const draftsStore = useDraftsStore();
      if (draftsStore.activeDraftName) {
        try {
          await draftsStore.deleteDraft(draftsStore.activeDraftName);
        } catch (err) {
          console.error("Failed to delete draft after payment:", err);
        }
      }
  ...
  function openPaymentDialog() {
    try {
      ...
      if (e.message === "SYSTEM_LOCKED_TIME_TAMPERING") {
        alert(
          "System locked due to time tampering. Please connect to the internet to sync and verify your time.",
        );
      } else if (e.message === "OFFLINE_LICENSE_EXPIRED") {
        alert(
          "Offline validity period expired. Please connect to the internet to sync and renew your license.",
        );
      } else {
        alert(e.message);
      }
