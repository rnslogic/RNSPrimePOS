...
export const useCustomerDisplayStore = defineStore("customerDisplay", () => {
  ...
  const outstanding = ref<CustomerOutstanding>({
    outstanding: 0,
    credit_limit: 0,
  });
  ...
  async function loadCustomerDetail(
    ...
    try {
      const [doc, addrData, contactData, invoiceData, outstandingData] =
        await Promise.all([
          call("pos_prime.api.customers.get_customer", {
          ...
          call("pos_prime.api.customer_profile.get_customer_pos_invoices", {
            customer: customerName,
            company,
          }).catch(() => []),
          call("pos_prime.api.customer_profile.get_customer_outstanding", {
            customer: customerName,
            company,
          }).catch(() => ({ outstanding: 0, credit_limit: 0 })),
        ]);
      ...
      outstanding.value = outstandingData || {
        outstanding: 0,
        credit_limit: 0,
      };
