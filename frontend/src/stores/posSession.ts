...
import { syncOfflineLicense } from "@/utils/security";
...
export const usePosSessionStore = defineStore("posSession", () => {
  ...
  async function checkOpeningEntry() {
    ...
    try {
      ...
      const data = await call("pos_prime.api.pos_session.check_opening_entry", {
      ...
      // If the backend call succeeded, we are online. Sync the offline validity period.
      const settingsStore = useSettingsStore();
      syncOfflineLicense(
        settingsStore.posProfile?.custom_offline_validity_days || 7,
      );
  ...
  async function createOpeningEntry(args: {
    ...
    try {
      ...
      // Successfully created opening entry online, update license sync
      const settingsStore = useSettingsStore();
      syncOfflineLicense(
        settingsStore.posProfile?.custom_offline_validity_days || 7,
      );
