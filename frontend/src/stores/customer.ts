...
export const useCustomerStore = defineStore("customer", () => {
  ...
  const loyaltyProgram = computed(
    () => loyaltyData.value?.loyalty_program ?? null,
  );
  const loyaltyPoints = computed(() => loyaltyData.value?.loyalty_points ?? 0);
  const maxRedeemableAmount = computed(
    () => loyaltyData.value?.max_redeemable_amount ?? 0,
  );

  async function searchCustomers(searchTerm: string, posProfile?: string) {
    try {
      if (!searchTerm) {
        // Try getting recent customers first
        let data = await call("pos_prime.api.customers.get_recent_customers", {
          pos_profile: posProfile || "",
          limit: 20,
        }).catch(() => []);

        // Fallback: pass "%%" to old search_customers API to bypass the len < 2 check
        if (!data || data.length === 0) {
          data = await call("pos_prime.api.customers.search_customers", {
            search_term: "%%",
            pos_profile: posProfile || "",
          }).catch(() => []);
        }
    ...
    try {
      ...
      if (data.loyalty_program) {
        try {
          const loyalty = await call(
            "pos_prime.api.loyalty.get_customer_loyalty",
            {
      ...
      try {
        const [addrData, contactData] = await Promise.all([
          call("pos_prime.api.addresses.get_customer_addresses", {
            customer: customerName,
          }),
          call("pos_prime.api.addresses.get_customer_contacts", {
            customer: customerName,
          }),
        ]);
  ...
  async function quickCreateCustomer(args: {
    ...
    try {
      const data = await call(
        "pos_prime.api.customers.quick_create_customer",
        args,
      );
      if (data) {
