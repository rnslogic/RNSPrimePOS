...
const loyaltyApplied = ref(false); // Store credit
...
const loyaltyRedemptionAmount = computed(() => {
	if (!loyaltyApplied.value || !redeemLoyalty.value || loyaltyPointsToRedeem.value <= 0)
		return 0;
	const cf = customerStore.loyaltyData?.conversion_factor || 0;
	return Math.min(loyaltyPointsToRedeem.value * cf, cartStore.roundedTotal);
});
...
const change = computed(
	() => Math.round(Math.max(0, paymentStore.totalPaid - effectiveGrandTotal.value) * 100) / 100,
);
const remaining = computed(
	() => Math.round(Math.max(0, effectiveGrandTotal.value - paymentStore.totalPaid) * 100) / 100,
);

const possibleWriteOff = computed(() => {
	const baseRemaining = Math.max(
		0,
		cartStore.roundedTotal - loyaltyRedemptionAmount.value - paymentStore.totalPaid,
	);
...
function onPaymentInput(e: Event) {
	...
	paymentStore.setPaymentAmount(paymentStore.activePaymentMethod, parseFloat(val) || 0);
}
...
async function doSubmit() {
	...
	try {
		...
		const itemsPayload = cartStore.items
		...
		const invoice = await paymentStore.submitInvoice({
			...
			custom_cashier_name:
				cashiersStore.currentCashier?.cashier_name || cashiersStore.currentCashier?.title,
			items: itemsPayload,
			payments: activePayments,
			taxes: settingsStore.posProfile?.taxes_and_charges || undefined,
			additional_discount_percentage:
				cartStore.pricingRuleDiscount?.type === "percentage"
					? cartStore.pricingRuleDiscount.value
					: cartStore.additionalDiscountPercentage || undefined,
			discount_amount:
				cartStore.pricingRuleDiscount?.type === "amount"
					? cartStore.pricingRuleDiscount.value
			...
			...(redeemLoyalty.value && loyaltyPointsToRedeem.value > 0 && customerStore.loyaltyData
				? {
						...
						loyalty_redemption_account:
							customerStore.loyaltyData.expense_account || undefined,
						loyalty_redemption_cost_center:
							customerStore.loyaltyData.cost_center || undefined,
					}
				: {}),
			...(writeOff > 0 ? { write_off_amount: writeOff } : {}),
			...(storeCreditAppliedAmount.value > 0
				? { store_credit_amount: storeCreditAppliedAmount.value }
				: {}),
			...Object.fromEntries(
		...
		const rawMessages: string[] =
			e.messages || (e.message ? [e.message] : ["Failed to submit invoice"]);
		errorMessages.value = rawMessages.map((msg: string) => msg.replace(/<[^>]*>/g, "").trim());
...
function onKeyDown(e: KeyboardEvent) {
	if (Date.now() - mountedTime < 100) return;
	if (e.key === "Escape") {
		paymentStore.closePaymentDialog();
		return;
	}
	// Do not intercept keystrokes if the user is typing in a native input or textarea.
	if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) {
		return;
	}
	// Handle Numpad / Keyboard numbers and decimals
	if (/^[0-9.]$/.test(e.key)) {
		pressKey(e.key);
		e.preventDefault();
	...
	<Teleport to="body">
		<Transition name="payment-overlay">
			...
			>
				...
				<!-- Dialog -->
				<div
					class="relative bg-white dark:bg-gray-900 w-full sm:max-w-md sm:rounded-2xl sm:shadow-2xl dark:sm:shadow-black/30 max-h-[100dvh] sm:max-h-[92vh] flex flex-col overflow-hidden rounded-t-2xl animate-slide-up sm:animate-scale-in"
				>
					<!-- Header with Grand Total -->
					<div
						class="bg-gradient-to-br from-gray-800 to-gray-900 text-white px-5 pt-4 pb-5 relative overflow-hidden"
					>
						<!-- Decorative circles -->
						<div class="absolute -top-8 -right-8 w-32 h-32 bg-white/5 rounded-full" />
						<div
							class="absolute -bottom-4 -left-4 w-20 h-20 bg-white/5 rounded-full"
						/>
						<div class="relative">
							...
							<div class="text-center mb-3">
								<div class="text-xs text-white/50 uppercase tracking-wider mb-1">
									{{ __("Grand Total") }}
								</div>
								...
								<div
									v-if="
										storeCreditAppliedAmount > 0 || loyaltyRedemptionAmount > 0
									"
									class="text-[10px] mt-0.5"
								>
									<span class="line-through text-white/30 mr-1">{{
										formatCurrency(cartStore.roundedTotal)
									}}</span>
									<span
										v-if="storeCreditAppliedAmount > 0"
										class="text-emerald-300 mr-1"
										>-{{ formatCurrency(storeCreditAppliedAmount) }}
										{{ __("credit") }}</span
									>
									<span
										v-if="loyaltyRedemptionAmount > 0"
							...
							>
								...
								<div
									v-if="remainingCreditLimit > 0"
									class="flex items-center gap-1 text-green-300"
								>
									<span
										>{{ __("Available Credit") }}:
										{{ formatCurrency(remainingCreditLimit) }}</span
									>
								</div>
							</div>
							...
							<div class="flex justify-between text-[10px] text-white/40 mt-1">
								<span
									>{{ formatCurrency(paymentStore.totalPaid) }}
									{{ __("paid") }}</span
								>
								<span v-if="remaining > 0"
									>{{ formatCurrency(remaining) }} {{ __("remaining") }}</span
								>
								<span v-else-if="change > 0" class="text-green-300"
									>{{ formatCurrency(change) }} {{ __("change") }}</span
								>
					...
					<div
						...
						class="mx-4 mt-3 px-3 py-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 rounded-xl text-xs font-medium space-y-1"
					>
						<div v-for="(msg, i) in errorMessages" :key="i">{{ msg }}</div>
					</div>
					<div
						v-else-if="error"
						class="mx-4 mt-3 px-3 py-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 rounded-xl text-xs font-medium"
					>
						{{ error }}
					</div>
					<!-- Scrollable Content -->
					<div class="flex-1 overflow-y-auto px-4 py-3 space-y-3">
						...
						<div class="flex flex-wrap gap-1.5 pb-0.5" role="tablist">
							...
							>
								...
								<span
									v-if="
										paymentStore.payments.find(
											(p) => p.mode_of_payment === pm.mode_of_payment,
										)?.amount > 0 &&
										paymentStore.activePaymentMethod !== pm.mode_of_payment
									"
						...
						<div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
							<div
								class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5 font-semibold"
							>
							...
							<input
								...
								min="0"
								placeholder="0"
								class="w-full text-2xl font-bold text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-900 rounded-lg px-3 py-2 border-2 transition-colors focus:outline-none"
								:class="
									activeAmount > 0
						...
						<!-- Cash Quick Amounts -->
						<div
							v-if="isCashMethod && cashShortcuts.length > 0"
						...
						>
							<label class="flex items-center gap-2.5 cursor-pointer">
								<input
									...
									class="w-4 h-4 rounded border-emerald-300 dark:border-emerald-600 text-emerald-600"
								/>
								<div class="flex-1">
									<div
										class="text-xs font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5"
									>
										<BadgeDollarSign :size="14" />
										{{ __("Apply Store Credit") }}
									</div>
									<div
										class="text-[10px] text-emerald-500 dark:text-emerald-400 mt-0.5"
									>
										{{ formatCurrency(customerStore.storeCredit) }}
										{{ __("available") }}
									</div>
								</div>
							</label>
							<div v-if="applyStoreCredit" class="mt-2.5 pl-6 space-y-2">
								<div class="flex items-center gap-2">
									<label
										class="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium shrink-0"
										>{{ __("Amount") }}:</label
									>
									<input
										...
										:max="
											Math.min(
												customerStore.storeCredit,
												cartStore.roundedTotal,
											)
										"
										class="flex-1 rounded-lg border border-emerald-200 dark:border-emerald-700 px-2.5 py-1.5 text-xs text-right focus:outline-none bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
									/>
								</div>
						...
						>
							<label class="flex items-center gap-2.5 cursor-pointer">
								<input
									...
									class="w-4 h-4 rounded border-violet-300 dark:border-violet-600 text-violet-600"
								/>
								<div class="flex-1">
									<div
										class="text-xs font-semibold text-violet-800 dark:text-violet-300 flex items-center gap-1.5"
									>
										<Award :size="14" /> {{ __("Redeem Loyalty Points") }}
									</div>
									<div
										class="text-[10px] text-violet-500 dark:text-violet-400 mt-0.5"
									>
										{{ customerStore.loyaltyPoints }}
										{{ __("pts available") }} ({{
											formatCurrency(customerStore.maxRedeemableAmount)
										}})
									</div>
								</div>
							</label>
							<div v-if="redeemLoyalty" class="mt-2.5 pl-6 space-y-2">
								<div class="flex items-center gap-2">
									<label
										class="text-[10px] text-violet-600 dark:text-violet-400 font-medium shrink-0"
										>{{ __("Points") }}:</label
									>
									<input
										...
										class="flex-1 rounded-lg border border-violet-200 dark:border-violet-700 px-2.5 py-1.5 text-xs text-right focus:outline-none bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
									/>
									<button
								...
								>
									<span class="text-violet-600 dark:text-violet-400">
										{{ loyaltyPointsToRedeem }} {{ __("pts") }} ×
										{{ customerStore.loyaltyData?.conversion_factor }}
									</span>
						...
						>
							<label class="flex items-center gap-2.5 cursor-pointer">
								<input
									...
									class="w-4 h-4 rounded border-amber-300 dark:border-amber-600 text-amber-600"
								/>
								<div class="flex items-center gap-1.5">
									...
									>
										{{ __("Write off") }}
										{{ formatCurrency(possibleWriteOff) }}
									</span>
								</div>
							</label>
						</div>
						...
						<div
							v-if="
								paymentStore.payments.filter((p) => p.amount > 0).length > 1 ||
								storeCreditAppliedAmount > 0
							"
							class="bg-gray-50 dark:bg-gray-800 rounded-xl p-3 space-y-1.5"
						>
							<div
								class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider font-semibold mb-1"
							>
								{{ __("Payment Summary") }}
							</div>
							<div
							...
							>
								<span
									class="text-gray-600 dark:text-gray-400 flex items-center gap-1.5"
								>
									<component
										:is="getMethodIcon(pm.mode_of_payment)"
										:size="12"
							...
							>
								<span
									class="text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5"
								>
									<BadgeDollarSign :size="12" /> {{ __("Store Credit") }}
								</span>
								<span
									class="font-semibold text-emerald-700 dark:text-emerald-300"
									>{{ formatCurrency(storeCreditAppliedAmount) }}</span
								>
							</div>
							<div
								class="border-t border-gray-200 dark:border-gray-700 pt-1.5 flex items-center justify-between text-xs font-bold"
							>
								...
								<span class="text-blue-600 dark:text-blue-400">{{
									formatCurrency(
										paymentStore.totalPaid + storeCreditAppliedAmount,
									)
								}}</span>
							</div>
						</div>
						...
						<div
							v-if="change > 0"
							class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3 text-center"
						>
							<div
								class="text-[10px] text-green-500 dark:text-green-400 uppercase tracking-wider font-semibold mb-0.5"
							>
								{{ __("Change Due") }}
							</div>
						...
						>
							...
							<div class="text-xs text-red-700 dark:text-red-400">
								<div class="font-semibold">
									{{ __("Credit limit would be exceeded") }}
								</div>
					...
					<!-- Fixed Submit Button -->
					<div
						class="px-4 py-3 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900"
					>
						<button
							@click="handleSubmit"
							:disabled="!canSubmit"
						...
						>
							<Loader2
								v-if="paymentStore.submitting"
								:size="18"
								class="animate-spin"
							/>
							<Check v-else :size="18" />
							{{
								paymentStore.submitting
									? __("Processing...")
									: __("Complete Payment")
							}}
						</button>
		...
		<div
			v-if="showPartialConfirm"
			class="fixed inset-0 z-[60] flex items-center justify-center p-4"
		>
			<div class="absolute inset-0 bg-black/40" @click="showPartialConfirm = false" />
			<div
				class="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl dark:shadow-black/30 w-full max-w-xs p-5 text-center"
			>
				<div
					class="w-10 h-10 bg-amber-50 dark:bg-amber-900/20 rounded-full flex items-center justify-center mx-auto mb-3"
				>
					<AlertTriangle :size="20" class="text-amber-500" />
				</div>
				<h4 class="text-sm font-bold text-gray-900 dark:text-gray-100 mb-1">
					{{ __("Partial Payment") }}
				</h4>
				<p class="text-xs text-gray-500 dark:text-gray-400 mb-1">
					{{ __("Paid") }}: {{ formatCurrency(paymentStore.totalPaid) }} /
					{{ formatCurrency(cartStore.roundedTotal) }}
				</p>
