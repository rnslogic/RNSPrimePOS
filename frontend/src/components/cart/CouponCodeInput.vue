...
<template>
	<div>
		...
		<div
			...
			class="flex items-center gap-2 bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-lg px-3 py-2"
		>
			<div
				class="w-5 h-5 bg-purple-100 dark:bg-purple-900/40 rounded flex items-center justify-center shrink-0"
			>
				<Tag :size="10" class="text-purple-600 dark:text-purple-400" />
			</div>
			<span class="text-xs font-bold text-purple-700 dark:text-purple-300 flex-1">{{
				displayCode || cartStore.couponCode
			}}</span>
			<button
				@click="removeCoupon"
				class="w-5 h-5 rounded flex items-center justify-center text-purple-400 dark:text-purple-500 hover:text-purple-600 dark:hover:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors"
			>
				<X :size="12" />
			</button>
		</div>
		...
		<div v-else class="flex gap-1.5">
			<div class="relative flex-1">
				<Tag
					class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-300 dark:text-gray-600 pointer-events-none"
					:size="11"
				/>
				<input
					v-model="input"
			...
			>
				...
				<Check v-else :size="10" /> {{ __("Apply") }}
			</button>
		</div>
		<div v-if="error" class="text-[10px] text-red-500 dark:text-red-400 mt-1 font-medium">
			{{ error }}
		</div>
	</div>
</template>
