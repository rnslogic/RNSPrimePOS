...
const totalWeight = computed(() => {
	return cartStore.items.reduce((sum, item) => {
		if (item.weight_per_unit) {
			return sum + item.weight_per_unit * item.qty;
		}
		return sum;
	}, 0);
});
...
<template>
	<div class="space-y-1">
		...
		>
			<span>{{ __("Total Qty") }}</span> <span>{{ cartStore.totalItems }}</span>
		</div>
		<!-- Net Total -->
		<div
			class="flex justify-between items-center text-gray-600 dark:text-gray-400 text-sm font-medium py-0.5"
		...
		>
			<span class="flex items-center gap-1">
				{{ __("Discount") }}
				<span
					class="px-1 py-0 bg-orange-100 dark:bg-orange-900/30 rounded text-[10px] font-semibold"
					>{{ cartStore.additionalDiscountPercentage }}%</span
				>
			</span>
			<span class="font-semibold"
				>-{{ formatCurrency(cartStore.subtotal - cartStore.netTotal) }}</span
			>
		</div>
		<div
		...
		>
			...
			<span class="font-semibold"
				>-{{ formatCurrency(cartStore.additionalDiscountAmount) }}</span
			>
		</div>
		...
		>
			<span class="flex items-center gap-1">
				...
				<span
					...
					>{{ cartStore.pricingRuleDiscount.value }}%</span
				>
			</span>
			<span class="font-semibold"
				>-{{ formatCurrency(cartStore.subtotal - cartStore.netTotal) }}</span
		...
		<div
			...
			class="flex justify-between text-gray-500 dark:text-gray-400 text-sm font-medium py-0.5"
		>
			<span>{{ __("Tax") }}</span> <span>{{ formatCurrency(cartStore.taxAmount) }}</span>
		</div>
		<!-- Tax calculating -->
		<div
		...
		>
			<Loader2 :size="10" class="animate-spin" /> <span>{{ __("Calculating...") }}</span>
		</div>
		<!-- Grand Total (ERPNext-style: bold, larger) -->
		<div
			class="flex justify-between items-center font-bold text-gray-900 dark:text-gray-100 pt-2 mt-1 border-t border-gray-200 dark:border-gray-700"
		>
		...
		<template v-if="cartStore.serverRoundingAdjustment !== 0">
			...
			<div
				class="flex justify-between items-center font-bold text-gray-900 dark:text-gray-100"
			>
