...
import { ShoppingCart, CreditCard, Pause, Check, X } from "lucide-vue-next";
...
function onUpdateQty(index: number, qty: number) {
	...
	const err = cartStore.updateQty(index, qty);
...
function onNumPadUpdate(value: number) {
	...
	if (numPadMode.value === "qty") {
		const err = cartStore.updateQty(cartStore.selectedItemIndex, value);
		if (err) {
			showStockError(err);
			keyboardInput.value = String(cartStore.items[cartStore.selectedItemIndex].qty);
		}
...
function handleCheckoutClick() {
	...
	if (!customerStore.customer) {
		customerStore.showValidationError = true;
		setTimeout(() => {
			customerStore.showValidationError = false;
		}, 3000);
		return;
	}
	openPayment();
...
const numpadInputRef = ref<HTMLInputElement | null>(null);
...
function handleKeydown(e: KeyboardEvent) {
	...
	// Do not intercept shortcuts if payment dialog is open
	if (paymentStore.showPaymentDialog) return;
	...
	if (e.key === "ArrowUp" || e.key === "ArrowDown") {
		...
		if (cartStore.selectedItemIndex === null) {
			cartStore.selectItem(e.key === "ArrowUp" ? cartStore.items.length - 1 : 0);
		} else {
			let newIdx =
				e.key === "ArrowUp"
					? cartStore.selectedItemIndex - 1
					: cartStore.selectedItemIndex + 1;
			if (newIdx < 0) newIdx = cartStore.items.length - 1;
			if (newIdx >= cartStore.items.length) newIdx = 0;
			cartStore.selectItem(newIdx);
		}
	} else if (e.key === "+" || e.key === "=" || e.key === "-") {
		e.preventDefault();
		if (cartStore.selectedItemIndex === null && cartStore.items.length > 0) {
			cartStore.selectItem(cartStore.items.length - 1);
		}
		if (cartStore.selectedItemIndex !== null) {
			const idx = cartStore.selectedItemIndex;
			const currentQty = cartStore.items[idx].qty;
			const newQty = e.key === "-" ? currentQty - 1 : currentQty + 1;
			if (newQty > 0) {
		...
		if (cartStore.selectedItemIndex !== null) {
			const idx = cartStore.selectedItemIndex;
			if (cartStore.items[idx]?.is_free_item) return;
			...
			if (!isTouchDevice) {
				...
				onNumPadUpdate(parseInt(e.key, 10));

				nextTick(() => {
					numpadInputRef.value?.focus();
				});
			}
		}
	} else if (e.key === "Enter") {
...
<template>
	<div class="flex flex-col h-full bg-white dark:bg-gray-900">
		...
		<!-- Cart items -->
		<div
			ref="cartScrollContainer"
			tabindex="0"
			@keydown="handleKeydown"
			class="flex-1 overflow-y-auto px-2 py-1 outline-none"
		>
			<div
				v-if="cartStore.items.length === 0"
		...
		<div class="border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
			...
			<div v-if="cartStore.items.length > 0" class="px-3 pt-1.5 space-y-1">
				<InvoiceDiscount />
				<CouponCodeInput v-if="false" />
				<InvoiceOptions v-if="false" />
			</div>
			...
			<!-- Action Buttons -->
			<div class="px-3 pb-12 sm:pb-2 flex gap-2">
				<button
				...
				<button
					...
					"
					title="Checkout (F3 or F9 or Enter)"
				>
					<CreditCard :size="16" />
					{{ __("Checkout") }}
				...
				<button
					...
					:disabled="cartStore.items.length === 0"
					class="py-2.5 px-4 rounded-lg text-sm font-bold transition-all duration-150 flex items-center justify-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 bg-gray-100 dark:bg-gray-800 text-red-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30"
