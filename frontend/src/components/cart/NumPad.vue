...
<template>
	<div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-2.5">
		...
		<div
			class="text-right text-xl font-bold text-gray-900 dark:text-gray-100 mb-2 px-3 py-1.5 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700"
		>
			{{ display }}
		</div>
		<div class="grid grid-cols-3 gap-1">
			<button
				...
				"
			>
				<Delete v-if="btn === 'DEL'" :size="16" /> <span v-else>{{ btn }}</span>
			</button>
		</div>
	</div>
</template>
