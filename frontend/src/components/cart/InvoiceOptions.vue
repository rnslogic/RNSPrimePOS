...
const expanded = ref(false); // Link targets
const salesPartners = ref<string[]>([]);
const projects = ref<string[]>([]);
const shippingRules = ref<string[]>([]);
const termsTemplates = ref<string[]>([]);
const paymentTermsTemplates = ref<string[]>([]); // Local form state synced to cart store
const form = computed({
...
onMounted(async () => {
	...
	try {
		...
		if (data) {
			...
			paymentTermsTemplates.value = (data.payment_terms_templates || []).map(
				(r: any) => r.name,
			);
		}
	} catch {
	...
	if (profile) {
		...
		if (profile.letter_head && !form.value.letter_head)
			defaults.letter_head = profile.letter_head;
		if (profile.select_print_heading && !form.value.select_print_heading)
			defaults.select_print_heading = profile.select_print_heading;
...
const billingAddresses = computed(() =>
	customerStore.addresses.map((a) => ({
		value: a.name,
		label: `${a.address_title} (${a.address_type})`,
	})),
);
...
watch(
	() => customerStore.selectedAddress,
	(val) => update("customer_address", val),
);
watch(
	() => customerStore.selectedShippingAddress,
	(val) => update("shipping_address_name", val),
);
watch(
	() => customerStore.selectedContact,
	(val) => update("contact_person", val),
);
...
const hasAnyOption = computed(() => {
	...
	return !!(
		...
		o.contact_person ||
		o.set_posting_time ||
		o.payment_terms_template ||
		o.tc_name
	);
});
const selectClass =
	"w-full text-xs border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-md px-2 py-1.5 focus:outline-none";
const inputClass =
	"w-full text-xs border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-md px-2 py-1.5 focus:outline-none";
</script>
<template>
	<div class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
		...
		>
			<span>
				{{ __("More Options") }}
				<span
					v-if="hasAnyOption && !expanded"
					class="ml-1 text-blue-500 dark:text-blue-400"
					>({{ __("configured") }})</span
				>
			</span>
		...
		<div
			v-if="expanded"
			class="px-3 pb-3 space-y-2.5 border-t border-gray-100 dark:border-gray-800"
		>
			<!-- Address & Contact -->
			<div
				v-if="customerStore.customer && customerStore.addresses.length > 0"
				class="space-y-2 pt-2"
			>
				<div class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase">
					{{ __("Address & Contact") }}
				</div>
				<div v-if="billingAddresses.length > 0">
					...
					<select
						...
						@change="
							customerStore.setSelectedAddress(
					...
					>
						...
						<option v-for="a in billingAddresses" :key="a.value" :value="a.value">
							{{ a.label }}
						</option>
					</select>
				</div>
				<div v-if="shippingAddresses.length > 0">
					...
					<select
						...
						@change="
							customerStore.setSelectedShippingAddress(
								($event.target as HTMLSelectElement).value || null,
							)
						"
						:class="selectClass"
					>
						<option value="">{{ __("None") }}</option>
						<option v-for="a in shippingAddresses" :key="a.value" :value="a.value">
							{{ a.label }}
						</option>
					</select>
				</div>
				<div v-if="contactOptions.length > 0">
					...
					<select
						...
						@change="
							customerStore.setSelectedContact(
								($event.target as HTMLSelectElement).value || null,
							)
						"
						:class="selectClass"
					>
						<option value="">{{ __("None") }}</option>
						<option v-for="c in contactOptions" :key="c.value" :value="c.value">
							{{ c.label }}
						</option>
					</select>
				</div>
			...
			<div class="pt-2">
				<div class="flex items-center gap-2">
					<input
						...
						@change="
							update('set_posting_time', ($event.target as HTMLInputElement).checked)
						"
						id="set-posting-time"
						class="w-3.5 h-3.5 rounded border-gray-300 dark:border-gray-600 text-blue-600"
					/>
					<label
						for="set-posting-time"
						class="text-[10px] text-gray-500 dark:text-gray-400"
						>{{ __("Override Posting Date/Time") }}</label
					>
				</div>
				<div v-if="form.set_posting_time" class="grid grid-cols-2 gap-2 mt-1.5">
					<div>
						<input
							...
							@input="
								update('posting_date', ($event.target as HTMLInputElement).value)
							"
							type="date"
							:class="inputClass"
						/>
					...
					<div>
						<input
							...
							@input="
								update('posting_time', ($event.target as HTMLInputElement).value)
							"
							type="time"
							step="1"
							:class="inputClass"
			...
			<div v-if="paymentTermsTemplates.length > 0">
				...
				>
					<option value="">None</option>
					<option v-for="pt in paymentTermsTemplates" :key="pt" :value="pt">
						{{ pt }}
					</option>
				</select>
			</div>
			...
			<div>
				...
				<textarea
					...
					class="w-full text-xs border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-md px-2 py-1.5 focus:outline-none resize-none placeholder-gray-400 dark:placeholder-gray-500"
				/>
			</div>
		</div>
	</div>
