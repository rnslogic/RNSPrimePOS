...
import { Monitor, Usb, ChevronDown } from "lucide-vue-next";
...
function openCustomerDisplay() {
	const sessionParam = sessionStore.openingEntry
		? `?session=${encodeURIComponent(sessionStore.openingEntry)}`
		: "";
	window.open(
...
<template>
	<div
		class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-56 overflow-hidden"
	>
		...
		<div class="p-2 space-y-1">
			...
			>
				<Monitor :size="16" /> Open Display Window
			</button>
			<!-- VFD section -->
			<div class="border-t border-gray-100 dark:border-gray-700 pt-1 mt-1">
				...
				<div v-if="isSupported && !isConnected" class="relative px-2.5 pb-1">
					...
					>
						<span>Baud: {{ baudRate }}</span> <ChevronDown :size="12" />
					</button>
					<div
						v-if="showBaudMenu"
				...
				<p
					v-if="!isSupported"
					class="px-2.5 py-2 text-xs text-gray-400 dark:text-gray-500"
				>
