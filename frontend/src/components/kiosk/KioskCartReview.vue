...
<template>
	<div class="flex h-full flex-col select-none" style="background: #0f172a">
		...
		<div
			class="px-6 py-5"
			style="background: #1e293b; border-bottom: 1px solid rgba(255, 255, 255, 0.06)"
		>
		...
		<div class="flex-1 overflow-y-auto px-6 py-5">
			...
			>
				<div class="text-center">
					<p class="text-xl" style="color: rgba(255, 255, 255, 0.3)">
						Your cart is empty
					</p>
					<button
			...
			<div v-else class="space-y-3">
				...
				>
					...
					<div class="flex-1 min-w-0">
						<div class="flex items-center gap-2">
							...
							<span
								...
								class="shrink-0 rounded-full px-2.5 py-0.5 text-xs font-bold uppercase"
					...
					<div v-if="!item.is_free_item" class="flex items-center gap-3">
						...
						<span class="w-10 text-center text-xl font-bold text-white">{{
							item.qty
						}}</span>
						<button
							class="flex h-12 w-12 items-center justify-center rounded-full text-xl font-bold text-white active:scale-90"
					...
					<div v-else class="flex items-center">
						<span class="text-xl font-bold" style="color: rgba(255, 255, 255, 0.3)"
							>×{{ item.qty }}</span
						>
					</div>
					<!-- Line total -->
					<p
						class="w-28 text-right text-xl font-bold"
						:style="item.is_free_item ? 'color: #4ade80;' : 'color: white;'"
					>
						{{ item.is_free_item ? "FREE" : formatCurrency(item.amount) }}
					</p>
					<!-- Remove -->
					<button
						v-if="!item.is_free_item"
		...
		<div
			...
			style="background: #1e293b; border-top: 1px solid rgba(255, 255, 255, 0.06)"
		>
				...
				<div class="mb-5 space-y-2">
					...
					>
						...
						<span class="text-white/70">{{
							formatCurrency(cartStore.taxAmount)
						}}</span>
					</div>
					<div
