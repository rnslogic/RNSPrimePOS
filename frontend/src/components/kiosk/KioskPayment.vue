...
<template>
	<div
		class="flex h-full flex-col items-center justify-center select-none"
	...
	>
		...
		<div class="w-full max-w-lg px-8 text-center">
			<!-- Amount -->
			<p class="mb-3 text-lg font-medium" style="color: rgba(255, 255, 255, 0.4)">
				Total Amount
			</p>
			...
			<!-- Discount summary -->
			<div
				v-if="cartStore.pricingRuleDiscount || cartStore.items.some((i) => i.is_free_item)"
				class="mt-3 mb-14 flex items-center justify-center gap-3 flex-wrap"
			>
				<span
				...
				>
					...
					{{
						cartStore.pricingRuleDiscount.type === "percentage"
							? cartStore.pricingRuleDiscount.value + "% discount applied"
							: "Discount applied"
					}}
				</span>
				<span
					v-if="cartStore.items.some((i) => i.is_free_item)"
					class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold"
			...
			<p class="mb-6 text-base font-medium" style="color: rgba(255, 255, 255, 0.35)">
			...
			<div class="space-y-4">
				...
				>
					...
					<svg
						class="ml-auto h-6 w-6 text-white/40"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
						stroke-width="2"
