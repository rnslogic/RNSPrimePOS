...
function onDigit(d: string) { if (phone.value.length < 15) { phone.value += d }
}
function onBackspace() { phone.value = phone.value.slice(0, -1)
}
function onClear() { phone.value = ''
}
function onLookup() { if (phone.value.length >= 3) { emit('lookup', phone.value) }
}
</script>
<template>
	<div
	...
	>
		<div class="w-full max-w-md px-8">
			...
			<div class="mb-8 text-center">
				...
				<p class="mt-2 text-sm" style="color: rgba(255, 255, 255, 0.35)">
			...
			<div v-if="matched" class="mb-5 flex items-center justify-center gap-3">
				<div
					...
					style="border-color: rgba(255, 255, 255, 0.1); border-top-color: #4ade80"
				/>
				<p class="text-sm" style="color: rgba(255, 255, 255, 0.4)">
					Taking you to scan items...
				</p>
			</div>
			<!-- Error -->
			<div
