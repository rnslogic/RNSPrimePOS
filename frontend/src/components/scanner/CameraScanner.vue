...
onMounted(async () => {
	try {
		...
		await html5Qrcode.start(
			...
			{
				fps: 10,
				// Removed qrbox to scan the full video feed, which is much better for wide 1D barcodes
			},
			(decodedText) => {
...
<template>
	<div class="fixed inset-0 z-50 bg-black flex flex-col pt-[env(safe-area-inset-top)]">
		<!-- Header -->
		<div class="flex items-center justify-between px-4 py-3 bg-black/80 z-10">
			<div class="flex items-center gap-2 text-white">
				<Camera :size="18" /> <span class="text-sm font-medium">Scan Barcode</span>
			</div>
			<button @click="close" class="text-white/80 hover:text-white"><X :size="20" /></button>
		</div>
		<!-- Camera view -->
