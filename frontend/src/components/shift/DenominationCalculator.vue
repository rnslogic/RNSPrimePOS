...
const grandTotal = computed(() => rows.value.reduce((sum, r) => sum + r.value * r.count, 0));
...
<template>
	<Teleport to="body">
		<div
			...
			role="dialog"
			aria-modal="true"
			:aria-label="__('Cash Denomination Count')"
		...
		>
			...
			>
				...
				<div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
					...
					<div v-if="notes.length > 0">
						...
						<div class="space-y-1.5">
							...
							>
								<span
									class="w-16 text-sm font-medium text-gray-700 dark:text-gray-300 text-right tabular-nums"
								>
								...
								<input
									...
									min="0"
									step="1"
									class="w-20 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-2 py-1.5 text-sm text-center focus:outline-none focus:border-blue-500 tabular-nums"
									@focus="($event.target as HTMLInputElement).select()"
								/>
								<span class="text-gray-400 dark:text-gray-500 text-sm">=</span>
								<span
									class="flex-1 text-sm text-right font-medium tabular-nums"
									:class="
										subtotal(row) > 0
				...
				<div class="border-t border-gray-200 dark:border-gray-700 px-5 py-4 space-y-3">
					<!-- Total bar -->
					<div
						class="flex items-center justify-between bg-blue-50 dark:bg-blue-900/30 rounded-lg px-4 py-3"
					>
						<span class="text-sm font-semibold text-blue-700 dark:text-blue-300">{{
							__("TOTAL")
						}}</span>
						<span
							class="text-lg font-bold text-blue-700 dark:text-blue-300 tabular-nums"
						>
