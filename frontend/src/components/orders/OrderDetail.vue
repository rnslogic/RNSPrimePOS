...
<template>
	<div class="h-full flex flex-col">
		...
		<template v-else>
			<!-- Header -->
			<div
				class="px-4 py-3 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900"
			>
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-xs text-gray-500 dark:text-gray-400">
							...
							{{ formatTime(order.posting_time) }}
						</p>
						<p v-if="order.owner" class="text-[10px] text-gray-400 dark:text-gray-500">
							by {{ order.owner }}
						</p>
					</div>
					<div class="flex gap-2">
						...
						>
							<Printer :size="12" /> {{ __("Print") }}
						</button>
						<button
							@click="showEmailDialog = true"
							class="px-3 py-1.5 text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors flex items-center gap-1"
						>
							<Mail :size="12" /> {{ __("Email") }}
						</button>
						<button
							v-if="canReturn()"
				...
				<div class="flex flex-wrap gap-1.5 mt-2">
					<span
						v-if="order.currency && order.currency !== 'USD'"
					...
					<span
						...
						class="text-[10px] bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-1.5 py-0.5 rounded"
					>
						{{ __("Coupon") }}: {{ order.coupon_code }}
					</span>
					<span
						v-if="order.loyalty_points > 0"
						class="text-[10px] bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 px-1.5 py-0.5 rounded"
					>
						{{ order.loyalty_points }} {{ __("pts redeemed") }}
					</span>
					<span
						v-if="order.consolidated_invoice"
						class="text-[10px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded"
					>
			...
			<div class="flex-1 overflow-y-auto px-4 py-3">
				...
				<div
					v-if="order.address_display || order.shipping_address || order.contact_display"
					class="mb-4 space-y-1.5"
				>
					<h4 class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">
						{{ __("Address & Contact") }}
					</h4>
					<div
						v-if="order.address_display"
					...
					<div
						v-if="order.contact_display"
						class="text-xs text-gray-600 dark:text-gray-400"
					>
						<span class="text-gray-400 dark:text-gray-500">{{ __("Contact") }}: </span
						>{{ order.contact_display }}
						<span v-if="order.contact_mobile"> ({{ order.contact_mobile }})</span>
					</div>
				</div>
				<!-- Document details -->
				<div
					v-if="order.po_no || order.project || order.sales_partner || order.remarks"
				...
				>
					...
					<div v-if="order.po_no" class="text-xs text-gray-600 dark:text-gray-400">
						<span class="text-gray-400 dark:text-gray-500"
							>{{ __("Customer PO") }}: </span
						>{{ order.po_no }}
						<span v-if="order.po_date"> ({{ formatDate(order.po_date) }})</span>
					</div>
					<div v-if="order.project" class="text-xs text-gray-600 dark:text-gray-400">
						<span class="text-gray-400 dark:text-gray-500">{{ __("Project") }}: </span
						>{{ order.project }}
					</div>
					<div
						v-if="order.sales_partner"
						class="text-xs text-gray-600 dark:text-gray-400"
					>
						<span class="text-gray-400 dark:text-gray-500"
							>{{ __("Sales Partner") }}: </span
						>{{ order.sales_partner }}
						<span v-if="order.commission_rate"> ({{ order.commission_rate }}%)</span>
						<span v-if="order.total_commission">
							= {{ formatCurrency(order.total_commission) }}</span
						>
					</div>
					<div v-if="order.remarks" class="text-xs text-gray-600 dark:text-gray-400">
						<span class="text-gray-400 dark:text-gray-500">{{ __("Remarks") }}: </span
						>{{ order.remarks }}
					</div>
				</div>
				<!-- Items -->
					...
					>
						<div class="flex-1 min-w-0">
							...
							<div class="text-xs text-gray-500 dark:text-gray-400">
								...
								<span
									v-else-if="item.discount_amount > 0"
									class="text-orange-500 dark:text-orange-400"
								>
								...
								<span
									v-if="
										item.uom && item.stock_uom && item.uom !== item.stock_uom
									"
							...
							<div
								...
								class="text-[10px] text-purple-400 dark:text-purple-500"
							>
				...
				<div v-if="order.taxes && order.taxes.length > 0" class="mt-4">
					<h4
						class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase mb-2"
					>
						{{ __("Taxes") }}
					</h4>
					<div
			...
			<!-- Totals -->
			<div
				class="px-4 py-3 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 space-y-1"
			>
				<div
					v-if="order.total_qty"
					class="flex justify-between text-xs text-gray-400 dark:text-gray-500"
				>
					<span>{{ __("Total Qty") }}</span> <span>{{ order.total_qty }}</span>
				</div>
				...
				<div
					v-if="order.total_taxes_and_charges > 0"
				...
				<div
					...
					class="flex justify-between text-sm text-orange-500 dark:text-orange-400"
				>
					<span>
						{{ __("Discount") }}
						<span v-if="order.additional_discount_percentage"
							>({{ order.additional_discount_percentage }}%)</span
						>
					</span>
					<span>-{{ formatCurrency(order.discount_amount) }}</span>
				</div>
				<div
					v-if="order.write_off_amount > 0"
					class="flex justify-between text-sm text-yellow-600 dark:text-yellow-400"
				>
					<span>{{ __("Write Off") }}</span>
					<span>{{ formatCurrency(order.write_off_amount) }}</span>
				</div>
				<div
					class="flex justify-between text-sm font-bold text-gray-900 dark:text-gray-100"
				>
					<span>{{ __("Grand Total") }}</span>
					<span>{{ formatCurrency(order.grand_total) }}</span>
				</div>
				<div
					v-if="order.rounded_total && order.rounded_total !== order.grand_total"
					class="flex justify-between text-xs text-gray-400 dark:text-gray-500"
				>
				...
				<div
					v-if="order.outstanding_amount > 0"
				...
				<div
					v-if="order.loyalty_points > 0"
					class="flex justify-between text-sm text-violet-500 dark:text-violet-400"
				>
					<span>{{ __("Loyalty Redeemed") }}</span>
					<span
						>{{ order.loyalty_points }} pts ({{
							formatCurrency(order.loyalty_amount)
						}})</span
					>
				</div>
				<!-- Base currency totals (multi-currency) -->
				<div
					v-if="order.conversion_rate && order.conversion_rate !== 1"
					class="pt-2 border-t border-gray-200 dark:border-gray-700 mt-2"
				>
					<div
						class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase mb-1"
					>
						{{ __("Company Currency") }}
					</div>
				...
				<!-- Weight -->
				<div
					v-if="order.total_net_weight"
					class="flex justify-between text-xs text-gray-400 dark:text-gray-500"
				>
					<span>{{ __("Total Weight") }}</span> <span>{{ order.total_net_weight }}</span>
				</div>
			</div>
		</template>
