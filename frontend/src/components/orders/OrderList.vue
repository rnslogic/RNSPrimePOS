...
<template>
	<div class="space-y-1.5">
		...
		<button
			...
			class="w-full text-left border rounded-lg p-3 transition-all duration-150"
			:class="
			...
			<div class="flex items-center justify-between">
				<div class="min-w-0 flex-1">
					...
					<div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
						{{ order.customer_name }} &middot; {{ formatDate(order.posting_date) }}
					</div>
					<div
						v-if="order.owner"
				...
				<div class="text-right shrink-0 ml-3">
					...
					>
						<RotateCcw v-if="order.is_return" :size="8" /> {{ statusLabel(order) }}
					</span>
				</div>
			</div>
		</button>
