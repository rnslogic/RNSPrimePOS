			...
			<div class="p-5 space-y-4">
					...
					>
						<div class="flex-1 min-w-0">
							...
							<div class="text-xs text-gray-500 dark:text-gray-400">
								{{ formatCurrency(item.rate) }} &middot; {{ __("Max") }}:
								{{ item.max_qty }}
							</div>
						</div>
						<div class="flex items-center gap-2">
							...
							<input
								...
								:min="0"
								:max="item.max_qty"
								class="w-16 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 px-2 py-1 text-sm text-center focus:outline-none"
							/>
						</div>
						<div
							class="w-20 text-right text-sm font-medium text-gray-800 dark:text-gray-200"
						>
				...
				<div>
					<label
						...
						>{{ __("Refund Method") }}</label
					>
					<select
						v-model="selectedPaymentMethod"
						class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:outline-none"
					>
						<option
				...
				<div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
					<div
						class="flex justify-between text-sm font-bold text-red-700 dark:text-red-400"
					>
