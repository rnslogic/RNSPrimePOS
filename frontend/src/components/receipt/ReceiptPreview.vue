...
function onKeyDown(e: KeyboardEvent) {
	...
	if (showEmailDialog.value) return; // Don't intercept if email dialog is open
	if (e.key === "Enter") {
		if (e.repeat) return;
		e.preventDefault();
		e.stopPropagation();
...
<template>
	<Teleport to="body">
		<div
			...
			role="dialog"
			aria-modal="true"
			:aria-label="__('Receipt')"
		>
			<div class="absolute inset-0 bg-black/40 backdrop-blur-sm no-print" />
			<div
				class="relative bg-white dark:bg-gray-900 w-full sm:rounded-2xl sm:shadow-2xl dark:sm:shadow-black/30 sm:max-w-sm max-h-[100dvh] sm:max-h-[90vh] flex flex-col overflow-hidden rounded-t-2xl animate-slide-up sm:animate-scale-in"
			>
				<!-- Success header (no-print) -->
				<div
					class="no-print bg-gradient-to-br from-green-500 to-green-600 text-white px-5 py-4 text-center relative overflow-hidden"
				>
					...
					<div class="relative">
						<div
							class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-2"
						>
							<Check :size="22" />
						</div>
				...
				<div class="flex-1 overflow-y-auto">
					<div class="p-5 text-center" id="receipt-content">
						...
						<div
							v-if="
								invoice.company_address_display ||
								settingsStore.posProfile?.company_address
							"
							class="text-[10px] text-gray-400 dark:text-gray-500 mb-1 leading-tight"
						>
							<span
								v-if="invoice.company_address_display"
								v-html="sanitizeHtml(invoice.company_address_display)"
							/>
						...
						<div class="text-[10px] text-gray-400 dark:text-gray-500 mb-4">
							...
							<span v-if="invoice.posting_time">{{ invoice.posting_time }}</span>
							<br />Cashier:
							{{
								invoice.custom_pos_cashier ||
						...
						<div
							...
							class="mb-3 px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-[10px] font-bold uppercase tracking-wider"
						>
							Return{{
						...
						<!-- Customer -->
						<div
							class="border-t border-dashed border-gray-200 dark:border-gray-700 pt-3 mb-3"
						>
							<div
								class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 font-semibold"
							>
								{{ __("Customer") }}
							</div>
							...
							<div
								v-if="invoice.address_display"
								class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-tight"
								v-html="sanitizeHtml(invoice.address_display)"
							/>
							<div
								v-if="invoice.contact_mobile || invoice.contact_email"
								class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5"
							>
								{{ invoice.contact_mobile
								}}{{ invoice.contact_mobile && invoice.contact_email ? " · " : ""
								}}{{ invoice.contact_email }}
							</div>
						</div>
						<!-- Items -->
						<div
							class="border-t border-dashed border-gray-200 dark:border-gray-700 pt-3"
						>
							<div
							...
							>
								<div class="text-left flex-1 pr-2">
									...
									<span
										...
										class="text-orange-500 dark:text-orange-400 ml-1 text-[10px] font-semibold"
									>
										-{{ item.discount_percentage }}%
									</span>
									<span
										v-else-if="(item.discount_amount || 0) > 0"
										class="text-orange-500 dark:text-orange-400 ml-1 text-[10px] font-semibold"
									>
										-{{ formatCurrency(item.discount_amount || 0) }}
									</span>
									<div
										v-if="item.serial_no || item.batch_no"
										class="text-[9px] text-gray-400 dark:text-gray-500 mt-0.5"
									>
										<span v-if="item.batch_no"
											>Batch: {{ item.batch_no }}</span
										>
										<span v-if="item.serial_no"
											>{{ item.batch_no ? " · " : "" }}SN:
											{{ item.serial_no }}</span
										>
									</div>
								...
								<span
									class="text-gray-800 dark:text-gray-200 font-semibold shrink-0"
								>
						...
						<!-- Totals -->
						<div
							class="border-t border-dashed border-gray-200 dark:border-gray-700 mt-3 pt-3 space-y-1"
						>
							<div
								v-if="invoice.total_qty"
							...
							<div
								v-if="invoice.net_total !== undefined"
							...
							<div
								...
								class="flex justify-between text-xs text-orange-600 dark:text-orange-400"
							>
								<span>
									{{ __("Discount") }}
									<span
										v-if="invoice.additional_discount_percentage"
										class="text-[10px]"
							...
							<!-- Coupon -->
							<div
								v-if="invoice.coupon_code"
								class="flex justify-between text-xs text-purple-600 dark:text-purple-400"
							>
								<span>Coupon: {{ invoice.coupon_code }}</span> <span></span>
							</div>
							<!-- Write-off -->
							<div
								v-if="invoice.write_off_amount > 0"
								class="flex justify-between text-xs text-amber-600 dark:text-amber-400"
							>
							...
							<!-- Grand Total -->
							<div
								class="flex justify-between text-sm font-bold text-gray-900 dark:text-gray-100 pt-1.5 border-t border-dashed border-gray-200 dark:border-gray-700"
							>
							...
							<!-- In Words -->
							<div
								v-if="invoice.in_words"
								class="text-[9px] text-gray-400 dark:text-gray-500 text-left italic pt-0.5"
							>
								{{ invoice.in_words }}
							</div>
							<!-- Payments -->
							<div
								class="pt-1.5 border-t border-dashed border-gray-200 dark:border-gray-700 space-y-0.5"
							>
								<div
							...
							<div
								v-if="invoice.change_amount > 0"
								class="flex justify-between text-xs text-green-600 dark:text-green-400 font-semibold"
							>
								<span>{{ __("Change") }}</span>
								<span>{{ formatCurrency(invoice.change_amount) }}</span>
							</div>
							<div
								v-if="invoice.outstanding_amount && invoice.outstanding_amount > 0"
								class="flex justify-between text-xs text-red-600 dark:text-red-400 font-semibold"
							>
							...
							<!-- Loyalty -->
							<div
								v-if="invoice.loyalty_points > 0"
								class="flex justify-between text-xs text-violet-600 dark:text-violet-400"
							>
								<span>{{ __("Loyalty Points Redeemed") }}</span>
								<span
									>{{ invoice.loyalty_points }} pts ({{
										formatCurrency(invoice.loyalty_amount)
									}})</span
								>
							</div>
							<!-- Sales Partner -->
							<div
								v-if="invoice.sales_partner"
								class="flex justify-between text-[10px] text-gray-400 dark:text-gray-500 pt-1"
							>
								<span>Sales Partner</span> <span>{{ invoice.sales_partner }}</span>
							</div>
						</div>
						<!-- PO Reference -->
						<div
						...
						<div
							...
							class="mt-2 text-[10px] text-gray-400 dark:text-gray-500 text-left"
						>
							{{ invoice.remarks }}
						</div>
				...
				<!-- Actions (no-print) -->
				<div
					class="no-print px-4 py-3 border-t border-gray-100 dark:border-gray-800 flex flex-col gap-2 bg-white dark:bg-gray-900"
				>
