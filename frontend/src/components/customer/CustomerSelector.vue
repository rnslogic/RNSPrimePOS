...
let debounceTimer: ReturnType<typeof setTimeout>; // Quick Entry state
...
function handleGlobalKeydown(e: KeyboardEvent) {
	if (e.key === "Escape" && showQuickEntry.value) {
		closeQuickEntry();
	}
}
...
async function onFocus() {
	if (results.value.length === 0 && !searchTerm.value) {
		results.value = await customerStore.searchCustomers("", sessionStore.posProfile);
	}
	showDropdown.value = results.value.length > 0;
}
...
function onEnter() {
	if (
		...
		selectCustomer(results.value[selectedIndex.value].name);
	}
}
...
<template>
	<div class="relative">
			...
			>
				...
				<div class="flex-1 min-w-0">
					...
					<div class="flex items-center gap-1.5 flex-wrap mt-0.5">
						...
						>
							<Phone :size="10" /> {{ customerStore.customer.mobile_no }}
						</span>
						<span
					...
					>
						...
						>
							<Tag :size="8" /> {{ customerStore.loyaltyPoints }} pts
						</span>
						<span
		...
		<div v-else>
			<div
				v-if="customerStore.showValidationError"
				class="text-[11px] font-bold text-red-500 mb-1 ml-1 animate-pulse"
			>
			...
			<div
				...
				:class="{
					'ring-2 ring-red-500 rounded-lg animate-shake':
						customerStore.showValidationError,
				}"
			>
				<Search
			...
			<Transition name="dropdown">
				...
				>
					<button
						...
						class="w-full text-left px-3 py-2.5 transition-colors first:rounded-t-lg last:rounded-b-lg"
						:class="
							selectedIndex === idx
		...
		<Teleport to="body">
			<Transition name="modal">
				...
				>
					...
					/>
					<div
						class="relative w-full max-w-sm mx-4 bg-white dark:bg-gray-900 rounded-xl shadow-2xl dark:shadow-black/50 overflow-hidden"
						style="color-scheme: light dark"
					>
						<!-- Header -->
						<div
							class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80"
						>
							<div class="flex items-center gap-2">
								<div
									class="w-7 h-7 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center"
								>
							...
							<button
								@click="closeQuickEntry"
								class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors !bg-transparent !border-0"
							>
								<X :size="16" />
							</button>
						</div>
						...
						<div v-if="quickEntryLoading" class="px-4 py-8 text-center">
							<Loader2
								:size="20"
								class="animate-spin text-blue-400 dark:text-blue-500 mx-auto"
							/>
						</div>
						<!-- Form -->
						<form
							...
							class="max-h-[70vh] overflow-y-auto overflow-x-hidden"
						>
							<div class="px-4 py-3 space-y-3">
								<!-- Error -->
								<div
									v-if="quickEntryError"
								...
								<div>
									<label
										class="text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1 block"
								...
								<div v-if="quickEntryOptions?.customer_types?.length > 1">
									<label
										class="text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1 block"
										>{{ __("Customer Type") }}</label
									>
									<select
										v-model="quickEntryForm.customer_type"
										class="qe-input"
									>
										<option
											v-for="t in quickEntryOptions.customer_types"
											:key="t"
											:value="t"
										>
											{{ t }}
								...
								<div v-if="quickEntryOptions?.customer_groups?.length > 0">
									...
									>
									<select
										v-model="quickEntryForm.customer_group"
										class="qe-input"
									>
										<option
											v-for="g in quickEntryOptions.customer_groups"
											:key="g"
											:value="g"
										>
											{{ g }}
								...
								<template v-if="quickEntryOptions?.quick_fields">
									<div
										v-for="f in quickEntryOptions.quick_fields.filter(
											(ff: any) => isExtraField(ff.fieldname),
										)"
										:key="f.fieldname"
									>
										<label
							...
							<div class="border-t border-gray-100 dark:border-gray-800">
								<button
									...
									class="w-full flex items-center justify-between px-4 py-2.5 text-[11px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors !bg-blue-50 dark:!bg-blue-900/20 !border-0"
								>
									{{ __("Primary Contact Details") }}
									<ChevronDown
										:size="14"
										class="transition-transform"
										:class="showContactSection ? 'rotate-180' : ''"
									/>
								</button>
								<div v-show="showContactSection" class="px-4 pb-3 space-y-3">
									...
									<div v-if="quickEntryForm.customer_type !== 'Company'">
										...
										>
										<input
											v-model="quickEntryForm.first_name"
											type="text"
									...
									<div v-if="quickEntryForm.customer_type !== 'Company'">
										...
										>
										<input
											v-model="quickEntryForm.last_name"
											type="text"
									...
									<div>
										...
										>
										<input
											v-model="quickEntryForm.email_id"
											type="email"
									...
									<div>
										...
										>
										<input
											v-model="quickEntryForm.mobile_no"
											type="tel"
							...
							<div class="border-t border-gray-100 dark:border-gray-800">
								...
								>
									...
									<ChevronDown
										:size="14"
										class="transition-transform"
										:class="showAddressSection ? 'rotate-180' : ''"
									/>
								</button>
								<div v-show="showAddressSection" class="px-4 pb-3 space-y-3">
									<div>
										...
										>
										<input
											v-model="quickEntryForm.address_line1"
											type="text"
									...
									<div>
										...
										>
										<input
											v-model="quickEntryForm.address_line2"
											type="text"
									...
									<div class="grid grid-cols-2 gap-3">
										<div>
											...
											>
											<input
												v-model="quickEntryForm.pincode"
												type="text"
									...
									<div>
										...
										<select v-model="quickEntryForm.country" class="qe-input">
											...
											<option
												v-for="c in quickEntryOptions?.countries || []"
												:key="c"
												:value="c"
											>
												{{ c }}
							...
							<div
								class="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center gap-2"
							>
								<button
									type="button"
									@click="closeQuickEntry"
									class="flex-1 text-sm font-medium py-2 rounded-md border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors !bg-white dark:!bg-gray-800"
								>
									{{ __("Cancel") }}
								</button>
								<button
...
.qe-input {
	@apply w-full !box-border text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md px-3 py-2 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-blue-400 dark:focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500;
