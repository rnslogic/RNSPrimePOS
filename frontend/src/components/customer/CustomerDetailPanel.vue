...
const { formatCurrency } = useCurrency(); // Editable fields
const editEmail = ref("");
const editMobile = ref("");
const editLoyaltyProgram = ref("");
const savingField = ref<string | null>(null); // Transactions
...
<template>
	...
	>
		...
		<div
			class="relative w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl dark:shadow-black/50 flex flex-col overflow-hidden pt-[env(safe-area-inset-top)]"
		>
			<!-- Header -->
			<div
			...
			<div v-if="customerStore.customer" class="flex-1 overflow-y-auto">
				...
				<div class="px-4 py-4 border-b border-gray-100 dark:border-gray-800">
					<div class="flex items-center gap-3">
						<div
							class="w-12 h-12 bg-blue-100 dark:bg-blue-900/40 rounded-xl flex items-center justify-center shrink-0"
						>
					...
					<div>
						...
						<div class="flex items-center gap-2">
							...
							<input
								...
								class="flex-1 text-sm text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md px-2.5 py-1.5 focus:outline-none focus:border-blue-400 transition-colors disabled:opacity-50 placeholder-gray-400 dark:placeholder-gray-500"
							/>
							<span
				...
				<div class="px-4 py-3">
					...
					<div v-else class="space-y-1">
						...
						>
							...
							<ExternalLink
								:size="12"
								class="text-gray-300 dark:text-gray-600 group-hover:text-blue-400 shrink-0"
							/>
						</button>
					</div>
				</div>
