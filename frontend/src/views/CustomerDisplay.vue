...
const returnOrder = ref<any>(null);
function loadDraft() {
...
function handleKeydown(e: KeyboardEvent) {
	if (e.key === "ArrowDown" || e.key === "ArrowUp") {
		const activeList = searchInput.value ? store.customers : store.recentCustomers;
		if (activeList.length === 0) return;
		const currentIndex = activeList.findIndex((c) => c.name === store.selectedCustomer?.name);
		if (e.key === "ArrowDown") {
			e.preventDefault();
			if (currentIndex < activeList.length - 1) {
				selectCustomer(activeList[currentIndex + 1].name);
			} else if (currentIndex === -1) {
				selectCustomer(activeList[0].name);
			}
...
<template>
	<AppShell
		@toggle-held-orders="showHeldOrders = true"
	...
	>
		<div class="h-full flex flex-col">
			...
			<ReturnDialog
				v-if="returnOrder"
				:order="returnOrder"
				@close="returnOrder = null"
				@success="returnOrder = null"
			/>
			<!-- Header -->
			<div
			...
			<div class="flex-1 flex overflow-hidden">
					...
					<div class="flex-1 overflow-y-auto p-3">
						...
						<div v-else-if="!searchInput">
							<div v-if="store.recentCustomers.length > 0">
								...
								>
									<Clock :size="12" /> Recent Customers
								</div>
								<div class="space-y-2">
									<button
									...
									>
										...
										<div
											class="flex items-center gap-3 mt-1.5 text-xs text-gray-400 dark:text-gray-500"
										>
											<span
												v-if="c.mobile_no"
												class="flex items-center gap-1"
											>
												<Phone :size="10" /> {{ c.mobile_no }}
											</span>
											<span
												v-if="c.email_id"
												class="flex items-center gap-1 truncate"
											>
												<Mail :size="10" /> {{ c.email_id }}
											</span>
											<span
						...
						<div
							v-else-if="store.customers.length === 0 && searchInput.length >= 2"
							class="flex flex-col items-center justify-center py-12 text-gray-400 dark:text-gray-500"
						>
				...
				>
					...
					<div
						...
						class="flex flex-col items-center justify-center h-full bg-gray-100 dark:bg-gray-900"
					>
						<div
						...
						<p class="text-sm mt-1.5 text-gray-400 dark:text-gray-500">
							{{ __("Search and select a customer to view details") }}
						</p>
					</div>
					<!-- Customer detail -->
					<div v-else class="max-w-3xl mx-auto p-4 lg:p-6 space-y-5">
						...
						>
							<div class="flex items-start justify-between">
								<div>
									...
									>
										<span
											v-if="store.selectedCustomer.customer_group"
											class="bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full"
										>
											{{ store.selectedCustomer.customer_group }}
										</span>
										<span
											v-if="store.selectedCustomer.territory"
											class="bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full"
										>
						...
						>
							...
							<div class="space-y-2 text-sm">
								<div
									v-if="store.selectedCustomer.mobile_no"
								...
								<div
									v-if="store.selectedCustomer.email_id"
									class="flex items-center gap-2 text-gray-600 dark:text-gray-400"
								>
								...
								<div
									v-if="store.selectedCustomer.tax_id"
									class="flex items-center gap-2 text-gray-600 dark:text-gray-400"
								>
									<FileText :size="14" class="text-gray-400" /> Tax ID:
									{{ store.selectedCustomer.tax_id }}
								</div>
								<div
									v-if="store.outstanding.credit_limit"
									class="flex items-center gap-2 text-gray-600 dark:text-gray-400"
								>
									<CreditCard :size="14" class="text-gray-400" /> Credit Limit:
									{{ formatCurrency(store.outstanding.credit_limit) }}
								</div>
								<div
									v-if="
										!store.selectedCustomer.mobile_no &&
								...
								>
									<div class="flex items-center gap-2 mb-1">
										...
										<span
											...
											class="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-1.5 py-0.5 rounded-full"
											>Primary</span
										>
										<span
											v-if="addr.is_shipping_address"
											class="text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded-full"
											>Shipping</span
										>
									</div>
									<div
										class="text-xs text-gray-500 dark:text-gray-400 ml-5"
										v-html="addr.display"
									/>
								</div>
							</div>
							...
							<div class="space-y-3">
								...
								>
									<div class="flex items-center gap-2 mb-1">
										...
										>
										<span
											v-if="contact.is_billing_contact"
											class="text-[10px] bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded-full"
											>Billing</span
										>
									</div>
									<div
										class="ml-5 space-y-0.5 text-xs text-gray-500 dark:text-gray-400"
									>
						...
						<!-- Loyalty -->
						<div
							v-if="store.loyaltyData"
							class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5"
						>
							<h4
								class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3"
							>
								<Star :size="14" class="inline mr-1 text-purple-500" /> Loyalty
								Program
							</h4>
							<div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
								<div class="flex justify-between">
									...
									<span class="font-medium text-gray-900 dark:text-gray-100">{{
										store.loyaltyData.loyalty_program
									}}</span>
								</div>
								<div class="flex justify-between">
									<span>Points Balance</span>
									<span class="font-medium text-gray-900 dark:text-gray-100">{{
										store.loyaltyData.loyalty_points
									}}</span>
								</div>
								<div class="flex justify-between">
									<span>Conversion Factor</span>
									<span class="font-medium text-gray-900 dark:text-gray-100">{{
										store.loyaltyData.conversion_factor
									}}</span>
								</div>
								<div class="flex justify-between">
									<span>Max Redeemable</span>
									<span class="font-medium text-gray-900 dark:text-gray-100">{{
										formatCurrency(store.loyaltyData.max_redeemable_amount)
									}}</span>
								</div>
							</div>
						</div>
								...
								<table class="w-full text-xs">
									...
									<tbody>
										...
										>
											...
											<td
												class="py-2 pr-3 text-right text-gray-600 dark:text-gray-400"
											>
												{{ inv.total_qty }}
											</td>
											<td
												class="py-2 pr-3 text-right font-medium text-gray-900 dark:text-gray-100"
											>
