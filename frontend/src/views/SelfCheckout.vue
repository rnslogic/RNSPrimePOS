...
const scanningRef = ref<
	| (ComponentPublicInstance & {
			showScannedFeedback: (name: string) => void;
			showScanError: (barcode: string) => void;
	  })
	| null
>(null);
...
const cashMethodAvailable = computed(() =>
	settingsStore.paymentMethods.some((m) => m.mode_of_payment.toLowerCase().includes("cash")),
);
...
const cashMethodName = computed(() => {
	const cash = settingsStore.paymentMethods.find((m) =>
	...
	if (!sessionName) {
		try {
			...
			if (openEntries && openEntries.length > 0) {
				...
				initError.value =
					"No open POS session found. Open a POS session first via POS Prime, then launch the kiosk.";
				initializing.value = false;
				return;
			}
		} catch {
			initError.value =
				"Could not check for open POS sessions. Please provide ?session=POS-OPE-XXXX in the URL.";
			initializing.value = false;
			return;
		}
	}
...
async function handleBarcodeScan(barcode: string) {
	...
	if (result) {
		...
		if (
			...
			result.is_stock_item &&
			result.actual_qty !== undefined &&
			result.actual_qty <= 0
		) {
	...
	try {
		...
		if (results && results.length > 0) {
			...
			phoneLookupError.value =
				"No account found with this number. You can continue as guest.";
		}
	} catch {
...
async function payByCard() {
	...
	if (result.success) {
		try {
			const methodName =
				cardMethodName.value || settingsStore.paymentMethods[0]?.mode_of_payment || "Cash";
...
<template>
	<div
		...
		style="background: #0f172a; touch-action: manipulation"
	>
		...
		<div v-if="initializing" class="flex h-full items-center justify-center">
			<div class="text-center">
				<div
					class="mx-auto mb-6 h-14 w-14 animate-spin rounded-full border-4"
					style="border-color: rgba(255, 255, 255, 0.08); border-top-color: #16a34a"
				/>
				<p class="text-lg" style="color: rgba(255, 255, 255, 0.4)">
					{{ __("Loading kiosk...") }}
				</p>
			</div>
		</div>
		...
		<div v-else-if="initError" class="flex h-full items-center justify-center p-8">
			<div
				class="max-w-md rounded-2xl p-8 text-center"
