...
let searchTimeoutTimer: ReturnType<typeof setTimeout> | null = null; // Animated time display for idle screen
const currentTime = ref("");
let clockInterval: ReturnType<typeof setInterval> | null = null; // Scope channel by session (POS Opening Entry) from URL query param
	...
	<div class="pole">
		...
		<Transition name="fade" mode="out-in">
			<div v-if="state === 'idle'" key="idle" class="screen idle-screen">
				...
				<div class="numpad-section">
					...
					<div class="numpad-display" :class="{ 'numpad-display--empty': !mobileInput }">
						...
						<span v-if="mobileInput" class="numpad-display-value">{{
							mobileInput
						}}</span>
					...
					<Transition name="slide-up">
						...
						<div
							v-else-if="mobileStatus === 'not_found'"
							class="m-status m-status--err"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
					...
					<div class="numpad-grid">
						<button
							...
							class="numpad-key"
							@click="numpadPress(n)"
							:disabled="mobileStatus === 'searching'"
						>
							{{ n }}
						</button>
						<button
							class="numpad-key numpad-key--fn"
							@click="numpadClear"
							:disabled="mobileStatus === 'searching'"
						>
							C
						...
						<button
							class="numpad-submit"
							@click="submitMobile"
							:disabled="!mobileInput.trim() || mobileStatus === 'searching'"
						>
			...
			<div v-else-if="state === 'cart' && cartData" key="cart" class="screen cart-screen">
				...
				<div class="cart-header">
					<div class="cart-header-left">
						<div v-if="companyLogo" class="cart-logo">
							<img :src="companyLogo" alt="" />
						</div>
						<div v-else-if="companyName" class="cart-logo cart-logo-text">
				...
				<div ref="poleCartItems" class="cart-items">
					<TransitionGroup name="item-list" tag="div">
						<div
							v-for="(item, i) in cartData.items"
							:key="item.item_name + '-' + i"
							class="cart-item"
							:class="{ 'cart-item--free': item.is_free_item }"
						>
							<div class="cart-item-info">
								<div class="cart-item-name-row">
									<span class="cart-item-name">{{ item.item_name }}</span>
									<span v-if="item.is_free_item" class="cart-free-badge"
										>FREE</span
									>
									<span v-else-if="item.pricing_rules" class="cart-promo-badge"
										>Promo</span
									>
								</div>
								<div v-if="item.is_free_item" class="cart-item-meta">
									Promotional item
								</div>
								<div v-else class="cart-item-meta">
									...
									<span
										...
										"
										class="cart-strike"
										>{{ fmt(item.price_list_rate, cartData.currency) }}</span
									>
									...
									<span
										...
										"
										class="cart-disc-badge"
										>-{{ fmt(item.discount_amount, cartData.currency) }}</span
									>
								</div>
							</div>
							<span
								class="cart-item-amount"
								:class="{ 'cart-item-amount--free': item.is_free_item }"
								>{{
									item.is_free_item
										? "FREE"
										: fmt(item.amount, cartData.currency)
								}}</span
							>
						</div>
					</TransitionGroup>
				...
				<div class="cart-footer">
					...
					<div
						v-if="cartData.pricingRuleDiscount"
						class="cart-summary-row cart-discount"
					>
						<span
							>Promo Discount{{
								cartData.pricingRuleDiscount.type === "percentage"
									? ` (${cartData.pricingRuleDiscount.value}%)`
									: ""
							}}</span
						>
						<span
							>-{{
								fmt(cartData.subtotal - cartData.netTotal, cartData.currency)
							}}</span
						>
					</div>
					<div
						v-else-if="cartData.discountValue > 0"
						class="cart-summary-row cart-discount"
					>
						<span
							>Discount{{
					...
					<div class="cart-total-row">
						...
						<span class="cart-total-amount">{{
							fmt(cartData.roundedTotal || cartData.grandTotal, cartData.currency)
						}}</span>
					</div>
				</div>
			</div>
			...
			<div v-else-if="state === 'payment'" key="payment" class="screen pay-screen">
				<div class="pay-top">
					<p v-if="paymentData?.customerName" class="pay-customer">
				...
				<div class="pay-center">
					...
					<div class="pay-total-block">
						...
						<span v-if="paymentData" class="pay-total-value">{{
							fmt(paymentData.grandTotal, paymentData.currency)
						}}</span>
						<div class="pay-total-bar"></div>
					</div>
					<!-- Payment breakdown -->
					<div
						v-if="paymentData && paymentData.payments.length > 0"
						class="pay-breakdown"
					>
						<div v-for="(p, i) in paymentData.payments" :key="i" class="pay-method">
							<div class="pay-method-left">
								...
								<div
									...
									:class="'pay-method-icon--' + paymentIcon(p.mode_of_payment)"
								>
									<svg
										v-if="paymentIcon(p.mode_of_payment) === 'cash'"
										xmlns="http://www.w3.org/2000/svg"
										viewBox="0 0 24 24"
										fill="none"
										stroke="currentColor"
									...
									<svg
										v-else-if="paymentIcon(p.mode_of_payment) === 'card'"
										xmlns="http://www.w3.org/2000/svg"
										viewBox="0 0 24 24"
										fill="none"
										stroke="currentColor"
									...
									<svg
										v-else-if="paymentIcon(p.mode_of_payment) === 'bank'"
										xmlns="http://www.w3.org/2000/svg"
										viewBox="0 0 24 24"
										fill="none"
										stroke="currentColor"
							...
							<span class="pay-method-amount">{{
								fmt(p.amount, paymentData!.currency)
							}}</span>
						</div>
						<!-- Change due -->
						<div v-if="paymentData.changeDue > 0" class="pay-change">
							...
							<span class="pay-change-amount">{{
								fmt(paymentData.changeDue, paymentData.currency)
							}}</span>
						</div>
					</div>
				</div>
			...
			<div v-else-if="state === 'complete'" key="complete" class="screen done-screen">
				...
				<p v-if="paymentData?.customerName" class="done-customer">
					{{ paymentData.customerName }}
				</p>
				<div v-if="completedTotal" class="done-total">
					<span class="done-total-label">Total Paid</span>
					<span class="done-total-value">{{
						fmt(completedTotal.grandTotal, completedTotal.currency)
					}}</span>
				</div>
