...
onMounted(async () => {
	try {
		...
		error.value =
			e.messages?.[0] ||
			e.message ||
			"Failed to fetch POS Profiles. Please check your connection or login again.";
	}
});
async function onProfileChange() {
	const profile = settingsStore.posProfiles.find((p) => p.name === selectedProfile.value);
	if (profile) {
...
<template>
	<div
		:class="[
			'bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4 relative',
			isDeskMode ? 'min-h-full' : 'min-h-screen',
		]"
	>
		<div
			class="w-full max-w-md bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6"
		>
			<div class="flex items-center gap-3 mb-6">
				<div
					class="flex items-center justify-center w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/30"
				>
			...
			<div class="space-y-4">
				<div v-show="settingsStore.posProfiles.length > 1">
					...
					<select
						...
						class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
					>
				...
				<div v-if="openingBalances.length > 0">
					...
					<div class="space-y-2">
						...
						>
							...
							<input
								v-focus-cash="isCash(balance.mode_of_payment)"
								v-model.number="openingBalances[index].opening_amount"
								type="number"
								min="0"
								step="0.01"
								class="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
								placeholder="0.00"
								@keyup.enter="openShift"
							/>
							<button
		...
		<DenominationCalculator
			:modelValue="
				denomCalcIndex >= 0 ? (openingBalances[denomCalcIndex]?.opening_amount ?? 0) : 0
			"
