...
const isNativeApp =
  !!(window as any).ipcRenderer || !!(window as any).Capacitor;

if (isNativeApp) {
  window.csrf_token = window.csrf_token || "dummy_token";
  window.frappe = window.frappe || { session: { user: "Guest" }, boot: {} };
}

const isFrappeEnvironment = !!(window as any).frappe?.boot;
const isStandalonePath =
  window.location.pathname.startsWith("/pos-prime") ||
  isNativeApp ||
  !isFrappeEnvironment;
const standaloneMount = isStandalonePath
