...
def get_pos_cashiers():
    """Get list of active POS Cashiers for PIN login."""
    validate_pos_access()
    # If the doctype doesn't exist yet, return an empty list safely
    if not frappe.db.exists("DocType", "POS Cashier"):
        return []
...
def close_shift(opening_entry, closing_amounts=None):
    ...
    try:
        ...
        # Log it to Frappe Error Log
        frappe.log_error("POS Close Shift Error", tb)
        ...
        # Instead of just raising, throw with the traceback so we can see it in UI
        frappe.throw(f"Error during submit: {str(exc)}\n\nTraceback: {tb}")
